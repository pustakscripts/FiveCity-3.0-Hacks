...
function startHack() {
	...
	for (row = 0; row < width; row++) {
		cells[row] = new Array();
		for (col = 0; col < height; col++) cells[row][col] = 0;
	}
	stepsCount = 0;
	squares = [];
...
function toggleLight(row, col) {
	if (row >= 0 && row < width && col >= 0 && col < width) {
		const square = squares.find(
			(sqr) =>
				parseInt(sqr.dataset.row) === row &&
				parseInt(sqr.dataset.column) === col
		);
...
function sweepStep() {
	...
	if (finished) {
		maxr = r;
		for (j = n; j < np; j++)
			for (i = r; i < m; i++) if (a(i, j) != 0) return false;
		return true;
	}
...
document
	...
	.addEventListener('mousedown', startMoving);
