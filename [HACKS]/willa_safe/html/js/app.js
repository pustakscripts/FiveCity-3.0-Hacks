...
function createDots() {
	...
	for (let i = 0; i < dotsAmount; i++) {
		dots.push({
			x: Math.floor(
				centerX + radius * Math.cos((2 * Math.PI * i) / dotsAmount)
			),
			y: Math.floor(
				centerY + radius * Math.sin((2 * Math.PI * i) / dotsAmount)
			),
		});
	}
}
...
function createLines() {
	...
	while (finish === false) {
		...
		if (
			lines.filter((line) => line.start === from && line.end === to).length > 0
		)
			continue;
		if (
			lines.filter((line) => line.start === to && line.end === from).length > 0
		)
			continue;
