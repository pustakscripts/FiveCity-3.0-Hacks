		...
		if (width > 0) {
			...
			if (w === 'remember') {
				hackTitle.textContent =
					'Dotrzyj z lewego górnego rogu, do prawego dolnego';
				hackInfoBox.style.display = 'none';
	...
	for (let i = 0; i < height * width; i++) {
		...
		el.onclick = function () {
			...
			if (lastPosition === 0) {
				...
				let jump = parseInt(
					document.querySelectorAll('.square')[lastPosition].dataset.value,
					10
				);
			...
			if (wrong === 3) {
				isOver = true;
				document
					.querySelectorAll('.breathing')
					.forEach((b) => b.classList.remove('breathing'));
				document
				...
				setTimeout(() => {
					$('html').hide();
					$.post(
						`https://${GetParentResourceName()}/gameFailed`,
						JSON.stringify({})
					);
				}, 3000);
			}
			...
			if (
				lastPosition === height * width - 1 &&
				document
					.querySelectorAll('.square')
					[lastPosition].classList.contains('good')
			) {
				gameWin();
			}
		};
